...
- Changed the primary entry point from `phyling.__main__` to `phyling.main`, using `run_cli` instead of the old `main` function.

- Made the internal function `phyling.lib.tree._fill_missing_taxon` publicly available.

- Improved the logger setup for better readability.

- Optimized the `SearchHitsManager` component.

- Updated dependency from iqtree2 to iqtree3.

- Use a symlink instead of copying the final_tree.nw generated by the tree module.

- Optimized the tool calling scripts under `phyling.external` to avoid violating the Liskov Substitution Principle.

- Made `muscle` and `raxml-ng` optional dependencies, allowing users to run phyling without these tools installed.

### Fixed

- Updated the pyhmmer dependency to version >= 0.12.0 to align with the latest behavior where some properties return str instead
  of bytes.
