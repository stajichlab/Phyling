...
#         captured = captured.split("Datasets available on local:")[1]
#         assert "tevenvirinae_odb10" in captured
...
#         assert "Local markerset bclasvirinae_odb10 is outdated. Updating from online database..." in caplog.text

#     @pytest.mark.dependency("database_update")
#     def test_download_already_up_to_date(self, caplog: pytest.LogCaptureFixture):
#         with caplog.at_level(logging.INFO):
#             download.download("bclasvirinae_odb10")
#         assert "Markerset bclasvirinae_odb10 already exists and is up to date." in caplog.text
